/**
 * WhatIsKelzaSection
 * Manages the "What is Kelza" section lifecycle.
 * Uses IntersectionObserver to progressively reveal child elements
 * as they scroll into the viewport.
 */

export class WhatIsKelzaSection {
  constructor() {
    this.element = document.getElementById('what-is-kelza-section');
    this.visible = false;
    this.observer = null;
    this.revealTargets = [];
  }
  ...
  _handleIntersections(entries) {
    entries.forEach((entry) => {
      if (entry.isIntersecting && entry.intersectionRatio >= 0.1) {
        entry.target.classList.add('what-is-kelza__element--revealed');
  ...
  /**
   * Reveal the what is kelza section by adding the visible modifier class.
   * Removes aria-hidden so assistive technology can perceive the content.
   */
  reveal() {
    if (!this.element) return;
    this.element.removeAttribute('aria-hidden');
    this.element.classList.add('what-is-kelza--visible');
    this.visible = true;
    this._setupObserver();
  }

  /**
   * Hide the what is kelza section by removing the visible modifier class.
   * Re-adds aria-hidden so assistive technology skips the content.
   */
  hide() {
    if (!this.element) return;
    this.element.setAttribute('aria-hidden', 'true');
    this.element.classList.remove('what-is-kelza--visible');
    this.visible = false;
  }

  /**
   * Check whether the what is kelza section is currently visible.
   * @returns {boolean}
   */
  isVisible() {
    return this.visible;
