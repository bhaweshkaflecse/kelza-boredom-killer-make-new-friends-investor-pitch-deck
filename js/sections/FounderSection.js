/**
 * FounderSection
 * Manages the "Why This Founder" section.
...
export class FounderSection {
  constructor() {
    this.element = document.getElementById('founder-section');
    this.visible = false;
    this.observer = null;
    this.revealTargets = [];
  }
  ...
  _handleIntersections(entries) {
    entries.forEach((entry) => {
      if (entry.isIntersecting && entry.intersectionRatio >= 0.15) {
        entry.target.classList.add('founder__element--revealed');
  ...
  /**
   * Reveal the founder section by adding the visible modifier class.
   * Removes aria-hidden so assistive technology can perceive the content.
   */
  reveal() {
    if (!this.element) return;
    this.element.removeAttribute('aria-hidden');
    this.element.classList.add('founder--visible');
    this.visible = true;
    this._setupObserver();
  }

  /**
   * Hide the founder section by removing the visible modifier class.
  ...
  hide() {
    ...
    this.element.classList.remove('founder--visible');
    ...
    this.revealTargets.forEach((el) =>
      el.classList.remove('founder__element--revealed')
    );
  ...
  /**
   * Check whether the founder section is currently visible.
   * @returns {boolean}
   */
  isVisible() {
    return this.visible;
