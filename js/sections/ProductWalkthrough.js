/**
 * ProductWalkthrough
 * Manages the product walkthrough section lifecycle.
 * Uses IntersectionObserver to progressively reveal child elements
 * as they scroll into the viewport.
 */

export class ProductWalkthrough {
  constructor() {
    this.element = document.getElementById('product-walkthrough-section');
    this.visible = false;
    this.observer = null;
    this.revealTargets = [];
  }
  ...
  _handleIntersections(entries) {
    entries.forEach((entry) => {
      if (entry.isIntersecting && entry.intersectionRatio >= 0.1) {
        entry.target.classList.add('product-walkthrough__element--revealed');
  ...
  /**
   * Reveal the product walkthrough section by adding the visible modifier class.
   * Removes aria-hidden so assistive technology can perceive the content.
   */
  reveal() {
    if (!this.element) return;
    this.element.removeAttribute('aria-hidden');
    this.element.classList.add('product-walkthrough--visible');
    this.visible = true;
    this._setupObserver();
  }

  /**
   * Hide the product walkthrough section by removing the visible modifier class.
   * Re-adds aria-hidden so assistive technology skips the content.
   */
  hide() {
    if (!this.element) return;
    this.element.setAttribute('aria-hidden', 'true');
    this.element.classList.remove('product-walkthrough--visible');
    this.visible = false;
  }

  /**
   * Check whether the product walkthrough section is currently visible.
   * @returns {boolean}
   */
  isVisible() {
    return this.visible;
