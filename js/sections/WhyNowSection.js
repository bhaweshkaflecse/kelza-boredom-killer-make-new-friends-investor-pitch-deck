/**
 * WhyNowSection
 * Manages the "Why Now" section lifecycle.
 * Uses IntersectionObserver to progressively reveal child elements
 * as they scroll into the viewport.
 */

export class WhyNowSection {
  constructor() {
    this.element = document.getElementById('why-now-section');
    this.visible = false;
    this.observer = null;
    this.revealTargets = [];
  }
  ...
  _handleIntersections(entries) {
    entries.forEach((entry) => {
      if (entry.isIntersecting && entry.intersectionRatio >= 0.15) {
        entry.target.classList.add('why-now__element--revealed');
  ...
  /**
   * Reveal the why-now section by adding the visible modifier class.
   * Removes aria-hidden so assistive technology can perceive the content.
   */
  reveal() {
    if (!this.element) return;
    this.element.removeAttribute('aria-hidden');
    this.element.classList.add('why-now--visible');
    this.visible = true;
    this._setupObserver();
  }

  /**
   * Hide the why-now section by removing the visible modifier class.
  ...
  hide() {
    ...
    this.element.classList.remove('why-now--visible');
    ...
    this.revealTargets.forEach((el) =>
      el.classList.remove('why-now__element--revealed')
    );
  ...
  /**
   * Check whether the why-now section is currently visible.
   * @returns {boolean}
   */
  isVisible() {
    return this.visible;
