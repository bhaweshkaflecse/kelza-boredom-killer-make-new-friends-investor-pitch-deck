/**
 * MarketSection
 * Manages the "Market Opportunity and Strategic Fit" section.
...
export class MarketSection {
  constructor() {
    this.element = document.getElementById('market-section');
    this.visible = false;
    this.observer = null;
    this.revealTargets = [];
  }
  ...
  _handleIntersections(entries) {
    entries.forEach((entry) => {
      if (entry.isIntersecting && entry.intersectionRatio >= 0.15) {
        entry.target.classList.add('market__element--revealed');
  ...
  /**
   * Reveal the market section by adding the visible modifier class.
   * Removes aria-hidden so assistive technology can perceive the content.
   */
  reveal() {
    if (!this.element) return;
    this.element.removeAttribute('aria-hidden');
    this.element.classList.add('market--visible');
    this.visible = true;
    this._setupObserver();
  }

  /**
   * Hide the market section by removing the visible modifier class.
  ...
  hide() {
    ...
    this.element.classList.remove('market--visible');
    ...
    this.revealTargets.forEach((el) =>
      el.classList.remove('market__element--revealed')
    );
  ...
  /**
   * Check whether the market section is currently visible.
   * @returns {boolean}
   */
  isVisible() {
    return this.visible;
