/**
 * ClosingSection
 * Manages the "Join the Journey" final chapter section.
 * Uses IntersectionObserver to progressively reveal child elements
 * as they scroll into the viewport.
 * Minimal JavaScript - the section is essentially static.
 */

export class ClosingSection {
  constructor() {
    this.element = document.getElementById('closing-section');
    this.visible = false;
    this.observer = null;
    this.revealTargets = [];
  }
  ...
  _handleIntersections(entries) {
    entries.forEach((entry) => {
      if (entry.isIntersecting && entry.intersectionRatio >= 0.15) {
        entry.target.classList.add('closing__element--revealed');
  ...
  /**
   * Reveal the closing section by adding the visible modifier class.
   * Removes aria-hidden so assistive technology can perceive the content.
   */
  reveal() {
    if (!this.element) return;
    this.element.removeAttribute('aria-hidden');
    this.element.classList.add('closing--visible');
    this.visible = true;
    this._setupObserver();
  }

  /**
   * Hide the closing section by removing the visible modifier class.
  ...
  hide() {
    ...
    this.element.classList.remove('closing--visible');
    ...
    this.revealTargets.forEach((el) =>
      el.classList.remove('closing__element--revealed')
    );
  ...
  /**
   * Check whether the closing section is currently visible.
   * @returns {boolean}
   */
  isVisible() {
    return this.visible;
