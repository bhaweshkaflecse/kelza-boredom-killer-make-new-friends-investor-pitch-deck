/**
 * BusinessSection
 * Manages the business section lifecycle.
...
export class BusinessSection {
  constructor() {
    this.element = document.getElementById('business-section');
    this.visible = false;
    this.observer = null;
    this.revealTargets = [];
  }
  ...
  _handleIntersections(entries) {
    entries.forEach((entry) => {
      if (entry.isIntersecting && entry.intersectionRatio >= 0.15) {
        entry.target.classList.add('business__element--revealed');
  ...
  /**
   * Reveal the business section by adding the visible modifier class.
   * Removes aria-hidden so assistive technology can perceive the content.
   */
  reveal() {
    if (!this.element) return;
    this.element.removeAttribute('aria-hidden');
    this.element.classList.add('business--visible');
    this.visible = true;
    this._setupObserver();
  }

  /**
   * Hide the business section by removing the visible modifier class.
  ...
  hide() {
    ...
    this.element.classList.remove('business--visible');
    ...
    this.revealTargets.forEach((el) =>
      el.classList.remove('business__element--revealed')
    );
  ...
  /**
   * Check whether the business section is currently visible.
   * @returns {boolean}
   */
  isVisible() {
    return this.visible;
