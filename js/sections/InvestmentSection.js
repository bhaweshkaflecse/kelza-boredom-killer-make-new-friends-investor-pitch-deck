/**
 * InvestmentSection
 * Manages the "Why Should I Invest Now" section.
 * Uses IntersectionObserver to progressively reveal child elements
 * as they scroll into the viewport.
 */

export class InvestmentSection {
  constructor() {
    this.element = document.getElementById('investment-section');
    this.visible = false;
    this.observer = null;
    this.revealTargets = [];
  }
  ...
  _handleIntersections(entries) {
    entries.forEach((entry) => {
      if (entry.isIntersecting && entry.intersectionRatio >= 0.15) {
        entry.target.classList.add('investment__element--revealed');
  ...
  /**
   * Reveal the investment section by adding the visible modifier class.
   * Removes aria-hidden so assistive technology can perceive the content.
   */
  reveal() {
    if (!this.element) return;
    this.element.removeAttribute('aria-hidden');
    this.element.classList.add('investment--visible');
    this.visible = true;
    this._setupObserver();
  }

  /**
   * Hide the investment section by removing the visible modifier class.
  ...
  hide() {
    ...
    this.element.classList.remove('investment--visible');
    ...
    this.revealTargets.forEach((el) =>
      el.classList.remove('investment__element--revealed')
    );
  ...
  /**
   * Check whether the investment section is currently visible.
   * @returns {boolean}
   */
  isVisible() {
    return this.visible;
